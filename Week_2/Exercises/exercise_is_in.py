...
# aStr. char will be a single character and aStr will be a string that is in alphabetical order. 
#
...
def isIn(char, aStr):
    ...
    '''
    # Base case 1: If aStr is empty, we can't find our char
    if len(aStr) == 0:
        return False
    
    # Base case 2: If aStr length is 1, compare char and aStr
    elif len(aStr) == 1:
        return char == aStr
    
    # Compare char value to the middle value of aStr
    
    # If char is larger than the middle character, throw out the first half of aStr
    elif char > aStr[len(aStr)//2]:
        return isIn(char, aStr[len(aStr)//2:] )
    
    # If char is smaller than the middle character, throw out the second half of aStr
    elif char < aStr[len(aStr)//2]:
        return isIn(char, aStr[:len(aStr)//2])
    
    # Base case 3: If char is not > or < middle of string, it must be equal, thus True
    else:
        return True
